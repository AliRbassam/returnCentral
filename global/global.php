...
function validEmail( $emasil ){
    return filter_var( @$email, FILTER_VALIDATE_EMAIL );
}


function echoYearDropDown($fieldName,$currentValue,$start,$end,$class="",$style=""){
...
function echoFileUpload($fieldName,$currentValue,$folder,$pageVars="",$required="",$image="",$file=""){
	global $entryId;
...
function echoDBCheckBox($fieldName,$currentValues,$table,$idColumn,$valueColumn,$extendedQuery="",$nextToEachOther=""){
	if($extendedQuery==""){
	...
	$float=""; $style="";
	if($nextToEachOther==1){$float="floatLeft"; $style="width:20%;";}
	while($row = fetchArray($result)){
		foreach($row as $key => $item){$$key = stripslashes(($row[$key]));}
		if(@count(@$currentValues)>0 && in_array($$idColumn,$currentValues)){
			echo "<div class='".$float."' style='".$style."'>
				<input checked type='checkbox' class='".$fieldName."' name='".$fieldName."[]' value=\"".$$idColumn."\"> ".$$valueColumn;
			echo"</div>";
		}else{
			echo "<div class='".$float."' style='".$style."'>
				<input type='checkbox' class='".$fieldName."' name='".$fieldName."[]' value=\"".$$idColumn."\"> ".$$valueColumn;
			echo"</div>";
		}
	}
...
function echoDBDropDown($fieldName,$currentValue,$table,$idColumn,$valueColumn,$extendedQuery="",$overrideQuery="",$class="",$style=""){
	if($extendedQuery==""){
		$result = runQuery("SELECT * FROM `".$table."` order by ".$valueColumn." ASC");
	}else{
		$result = runQuery("SELECT * FROM `".$table."` ".$extendedQuery." ");
...
function echoDBDropDown2($fieldName,$currentValue,$table,$idColumn,$valueColumn,$valueColumn2,$extendedQuery="",$overrideQuery="",$class="",$style=""){
	if($extendedQuery==""){
		$result = runQuery("SELECT * FROM `".$table."` order by ".$valueColumn." ASC");
	}else{
		$result = runQuery("SELECT * FROM `".$table."` ".$extendedQuery." ");
	...
	while($row = fetchArray($result)){
		foreach($row as $key => $item){$$key = stripslashes(($row[$key]));}
		if($$idColumn==$currentValue){
			echo "<option value=\"".$$idColumn."\" selected='selected'>".$$valueColumn." | ".$$valueColumn2."</option>
			";
		}else{
			echo "<option value=\"".$$idColumn."\">".$$valueColumn." | ".$$valueColumn2."</option>
			";
		}
		$i++;
	}
...
function uploadImage($var, $width, $height, $t_width, $t_height){
	...
	global $table, $entryId, $folder,$error;
	$image="";
	if(@$_FILES[$var]["name"]){
			//if an image was uploaded
			...
			if(!allowedExtension($_FILES[$var]['name'],$extensions)){
				...
				$temp->load($folder."originals/".$image);
				$temp->crop($width,$height);
				$temp->save($folder."images/".$image);

				$temp->load($folder."originals/".$image);
				$temp->crop($t_width,$t_height);
				$temp->save($folder."thumbs/".$image);
	...
	if($pages<2){
		...
		while($i<=$pages){
			if($i<$pages){
				//all pages but last
				if($i==$current){
						echo "<a class='currentPage' style='text-decoration:underline;'>$i</a> ";
				}else{
		...
		if($current<4){// 1 - 2 -3 - 4 - 5 Last
			...
			echo "<a class='otherPage' href='$basePage?page=$i$args'>$i</a> ";
			$i++;
			echo "<a class='currentPage' style='text-decoration:underline;'>$i</a> ";	// current page
			$i++;
			echo "<a class='otherPage' href='$basePage?page=$i$args'>$i</a> ";
			$i++;
			echo "<a class='otherPage' href='$basePage?page=$i$args'>$i</a>";
